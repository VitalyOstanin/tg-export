...
Флаг `--json` для машиночитаемого вывода поддерживают команды `list`, `account list`, `auth check`, `state show` и `tg info`. В этом режиме в stdout печатается только JSON.
