...
Отдельно ставить ничего не нужно: `python-socks` входит в зависимости пакета, поэтому
и `uv tool install tg-export`, и `uv sync` в клоне репозитория дают работающий прокси.
Прежнее написание `tg-export[proxy]` принимается и ничего не добавляет.

Если пакет всё же отсутствует в окружении, `tg-export` отказывается стартовать с
настроенным прокси, а не подключается напрямую: Telethon в такой ситуации молча
игнорирует настройку и раскрывает реальный IP.

### min_free_space

Минимальное свободное место на диске. Экспорт завершается ошибкой, когда свободного места
становится меньше указанного значения: продолжать нечем, а состояние уже сохранено -- следующий
запуск продолжит с той же точки. Приостановки с ожиданием освободившегося места нет.
...
| `media.concurrent_downloads` | число (1-5) | `3` | Параллельные загрузки внутри чата. Потолок -- осторожность: выше пяти экспорт не проверялся, а загрузки делят одно соединение и держат каждая свой буфер |
...
### Исключение: `skip` у типа действует внутри папки

Правило типа с `skip: true` запрещает чат, а не описывает его медиа, поэтому оно применяется и
к чатам папки -- раньше, чем правила самой папки. Всё остальное, что папка говорит о медиа,
по-прежнему сильнее правила типа.

Пример: канал лежит в папке `Работа`, у папки задан свой раздел `media`, а `type_rules` содержит
`public_channel: {skip: true}`. Канал будет пропущен, хотя папка стоит выше по списку: `skip`
типа отвергает чат до того, как раздел `media` папки получает слово. Чтобы выгружать такой
канал, снимите `skip` у типа и опишите исключение отдельным правилом в `chats` или в
`folders.Работа.chats`.
